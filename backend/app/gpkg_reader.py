...
def read_gpkg(data: bytes) -> list[GpkgLayer]:
    ...
    try:
        ...
        try:
            ...
            pass  # transient Windows handle — a leaked temp file must not fail the upload
...
_LABEL_FIELDS = (
    "dsu_num", "dsu_name", "unit_name", "unit", "deal_name", "dealname",
    "deal", "name", "prospect", "label", "title", "tract_id", "deal_id", "id",
)
    ...
    for table, geom_col, srs_id in rows:
        ...
        if feats:
            layers.append(GpkgLayer(
                table_name=table, srs_id=srs_id,
                crs=_resolve_crs(conn, srs_id), features=feats))
    if not layers:
...
def _read_features(conn: sqlite3.Connection, table: str, geom_col: str) -> list[GpkgFeature]:
    ...
    for i, row in enumerate(conn.execute(f"SELECT {sel} FROM {_quote_ident(table)}")):
        ...
        attrs = {c: _jsonable(v) for c, v in zip(attr_cols, row[off + 1:])}
