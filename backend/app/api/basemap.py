"""Serve the self-hosted Protomaps PMTiles basemap with HTTP range support.

PMTiles reads via byte-range requests, so this must honor the Range header
(RFC 7233) and answer 206 + Content-Range. Reuses the permian.pmtiles file
already vendored under permian_type_curve/infra/basemap (see PMTILES_PATH).
"""
...
def _serve_geojson(path: Path) -> Response:
    if not path.is_file():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"GeoJSON not found at {path}.",
        )
    return FileResponse(
        path,
...
@router.get("/blocks.geojson")
async def serve_blocks() -> Response:
    """TX GLO + NM blocks (survey grid). Static; covers both basins."""
    return _serve_geojson(settings.blocks_geojson_path)


@router.get("/sections.geojson")
async def serve_sections() -> Response:
    """TX GLO + NM sections; section number in LEVEL3_SUR. Static; both basins."""
